...
data_file <-here("examples", "TESLA_PB", "peptides_MHC.txt")
...
for (i in 1:nrow(peptides_MHC)){
  ...
  file_fasta <- here("examples", "TESLA_PB", paste(pep, ".fasta", sep = ""))
  ...
  final_output_file <- here("examples", "TESLA_PB", paste(pep, ".csv", sep = ""))
