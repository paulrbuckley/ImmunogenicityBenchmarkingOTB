...
author: paulbuckley, RDM, University of Oxford.
...
- Peptides are tested per allele.
...
# Files are output from IEDB per allele and saved with the allele in the file name. Allele is not output in the data, so the below extracts alllele information into a column from the file name
...
- Seperates peptides by HLA. Runs analysis into /NETTEPI_OTB folder.
...
# iPred
## Run OTB
- The below .R script is taken from Shughay's repo.
- Only modifications made are the last three lines, which uses the trained model to predict P(immunogenicity) for our CoV2 peptides of interest.
...
### Run
- Code to run 'PRIME'
- Please see PRIME repository for installing and executing this model. MixMHCpred is required.
- 'RESULTS_OUTPUT' location will need to be changed
```{}

TEST_DATA_LOCATION="SARS_COV_2_DATASET/PRIMEOTB/"
# Cant have spaces in full path so save data to ~Documents/PRIMEDATA/x
...
```{,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,dpi=300}
gl <- lapply(list(roc_AUC,pr_AUC), ggplotGrob)
library(grid)
